...
void SensitiveDetector::EndOfEvent(G4HCofThisEvent *)
{
    ...
    const G4int totalEvents   = 1000000;
    const G4int printInterval = 100000;   // To print every 10000000

    if ((eventID + 1) % printInterval == 0)
    {
        G4double progress = 100.0 * (eventID + 1) / totalEvents;
        G4cout << "Progress: " << progress << "%  |  Event " << eventID + 1 << " deposited: " << fTotalEnergyDeposited << " MeV." << G4endl;  // We print in the terminal the total energy deposited per event as soon as it finishes    
    }
