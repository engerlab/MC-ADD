...
{ 
    ...
    G4Material   *SrcMat = nist->FindOrBuildMaterial("G4_Cs");    // Source's material
    ...
    Pos3 = G4ThreeVector(0*mm, 0*mm, 17.5*mm);  // Detector
    ...
    G4double SourceX = 2/2.*mm;
    G4double SourceY = 2/2.*mm;
    G4double SourceZ = 2/2.*mm;
    ...
    G4double DetectorX = 15/2.*mm;
    G4double DetectorY = 15/2.*mm;
    G4double DetectorZ = 15/2.*mm;
    ...
    // ::: PMMA_Block :::
    //auto mesh1 = CADMesh::TessellatedMesh::FromOBJ(basePath + "PMMA_Block.obj");
    auto mesh1 = CADMesh::TessellatedMesh::FromSTL(basePath + "PMMA_Block.stl");
    ...
    auto PMMA_Block = mesh1->GetSolid();
    auto PMMA_Block_log = new G4LogicalVolume(PMMA_Block, PMMA, "PMMA_Block_log", 0, 0, 0);
                        new G4PVPlacement(Rotation, Pos1, PMMA_Block_log, "PMMA_Block", World_log, 0, checkOverlaps); // ...here.
    ...
    // ::: PMMA :::
    G4VisAttributes *PMMAVisAtt = new G4VisAttributes(G4Color(0.0, 1.0, 1.0, 0.2)); // Cyan
    PMMAVisAtt->SetForceSolid(true);
    PMMA_Block_log->SetVisAttributes(PMMAVisAtt);
