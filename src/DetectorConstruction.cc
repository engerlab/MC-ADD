...
#include "CADMesh.hh"               // To import Meshfiles
...
{ 
    ...
    G4Material   *SrcMat = nist->FindOrBuildMaterial("G4_PLEXIGLASS");    // Source's material
    G4Material   *DetMat = nist->FindOrBuildMaterial("G4_SODIUM_IODIDE");  // Detector's Active Volume material
    ...
    // :::  Positions  :::
    Pos1 = G4ThreeVector(0*mm, 0*mm, 0*mm);  // World position
    ...
    G4double SourceOutRad    = 2.375*mm;
    G4double SourceThickness = 1/2.*mm;
    ...
    // ::::::::::::::::::::::
/*
    G4double DetectorX = 1/2.*mm;
    G4double DetectorY = 1/2.*mm;
    G4double DetectorZ = 1/2.*mm;
  ...
  */          
    ...
    /* In this section you can add more geometries using Meshfiles in .obj or .stl format.
     * !!! Coppy and uncomment the following section if you want to model the Meshfiles, or if you wich to add more Meshgeometries. !!!
     * !!! The 'X' represents the number of Meshgeometries, 'Name' is the name of the Meshfile, and 'MATERIAL' should be defined before compiling. !!!
     *
     * The Meshfiles are added throught the GUI, but if you wish to add them manually, here is the detailed processs on how to proceed:
     *     1) Import .obj, or .stl geometry using:
     *
     *        auto meshX = CADMesh::TessellatedMesh::FromOBJ("NAME.obj");  // If you want to add more Meshfiles, keep using mesh2,3,4, etc. Remember to write the correct file name!
     *        auto meshx = CADMesh::TessellatedMesh::FromSTL("Name.stl");  // stl geometries MUST BE SAVED IN ASCII STL
     *
     *     2) Scale the geometry. Recommended to set it to 1 to keep your original dimensions
     *
     *        meshX->SetScale(1); // For more geoemtries use mesh2, mesh3, etc.
     *
     *     3) Setting an offset in the geometry position. Modify it as needed, but adding mesh2, mesh3, etc for more geometries.
     *
    ...
    //std::string basePath = "/home/victor/Desktop/GATE/MC-ADD/CAD_Files/Obj/";  // For .obj files
    std::string basePath = "/home/victor/Desktop/GATE/MC-ADD/CAD_Files/Stl/";  // For .stl files. MUST BE IN ASCII STL

       
    // ::: AlHousing :::
    //auto mesh1 = CADMesh::TessellatedMesh::FromOBJ(basePath + "AlHousing.obj");
    auto mesh1 = CADMesh::TessellatedMesh::FromSTL(basePath + "AlHousing.stl");

    mesh1->SetScale(1);
    mesh1->SetOffset(x, y, z);
    mesh1->SetOffset(G4ThreeVector(x, y, z));

    Rotation = new G4RotationMatrix();   // Rotate your geometry accordingly
    Rotation->rotateX(90.*deg);
    Rotation->rotateY(0.*deg);
    Rotation->rotateZ(0.*deg);

    auto AlHousing = mesh1->GetSolid();
    auto AlHousing_log = new G4LogicalVolume(AlHousing, Al, "AlHousing_log", 0, 0, 0);
                        new G4PVPlacement(Rotation, Pos1, AlHousing_log, "AlHousing", World_log, 0, checkOverlaps); // ...here.
            
    // ::: Rod :::
    //auto mesh2 = CADMesh::TessellatedMesh::FromOBJ(basePath + "Rod.obj");
    auto mesh2 = CADMesh::TessellatedMesh::FromSTL(basePath + "Rod.stl");
    ...
    auto Rod = mesh2->GetSolid();
    auto Rod_log = new G4LogicalVolume(Rod, PMMA, "Rod_log", 0, 0, 0);
                        new G4PVPlacement(Rotation, Pos1, Rod_log, "Rod", World_log, 0, checkOverlaps); // ...here.
            
    // ::: Shielding :::
    //auto mesh3 = CADMesh::TessellatedMesh::FromOBJ(basePath + "Shielding.obj");
    auto mesh3 = CADMesh::TessellatedMesh::FromSTL(basePath + "Shielding.stl");
    ...
    auto Shielding = mesh3->GetSolid();
    auto Shielding_log = new G4LogicalVolume(Shielding, PbMat, "Shielding_log", 0, 0, 0);
                        new G4PVPlacement(Rotation, Pos1, Shielding_log, "Shielding", World_log, 0, checkOverlaps); // ...here.
            
    // ::: NaI :::
    //auto mesh4 = CADMesh::TessellatedMesh::FromOBJ(basePath + "NaI.obj");
    auto mesh4 = CADMesh::TessellatedMesh::FromSTL(basePath + "NaI.stl");
    ...
    auto DetectorStl = mesh4->GetSolid();    
